...
import cz.droidboy.worktime.wifi.range.Proximity;
...
public class ProximityUtils {
    ...
    public static ScanFilter prepareFilter(Context context) {
        ...
        String mac = prefs.getString(SettingsActivity.KEY_PREF_BSSID, null);
        String ssid = prefs.getString(SettingsActivity.KEY_PREF_SSID, null);
        String[] parsedChannels = prefs.getString(SettingsActivity.KEY_PREF_CHANNELS, "").split("[,|, ]");
        ...
        if (mac == null && ssid == null && channels == null) {
            return null;
        } else {
            return new ScanFilter(mac, ssid, channels, null);
        }
    }
